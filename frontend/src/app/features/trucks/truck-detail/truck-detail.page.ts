...
@Component({
  ...
  imports: [DatePipe, DecimalPipe, HlmAccordionImports, HlmBadgeImports, HlmProgressImports, HlmSpinnerImports, NgIcon],
  viewProviders: [provideIcons({ lucideExternalLink, lucideMapPin, lucideTruck, lucideUser })],
  template: `
    <div class="flex h-full flex-col gap-4 overflow-y-auto rounded-md bg-card p-6">
      @switch (status()) {
        @case ('loading') {
