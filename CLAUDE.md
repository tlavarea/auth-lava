...
- **`sw-expedited/`** — Spring Boot 4.1 / Java 25 service for application functionality, sitting behind auth-lava rather than duplicating its login/session logic. It trusts auth-lava's JWTs directly: `SecurityConfiguration` fetches auth-lava's public key from its JWKS endpoint (`/.well-known/jwks.json`) via Spring Security's OAuth2 resource server support, and reads the bearer token from the same `ACCESS_TOKEN` cookie auth-lava issues (`CookieBearerTokenResolver`) rather than an `Authorization` header. No datasource of its own yet — add one when a feature actually needs to persist something.
...
- sw-expedited: `cd sw-expedited && ./mvnw verify` (build + test), `./mvnw spring-boot:run` (run locally; needs auth-lava running on `localhost:8080` for JWKS verification, no `.env` required)
