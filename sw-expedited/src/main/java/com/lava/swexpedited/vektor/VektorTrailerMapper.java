 ...
 * {@code " - "} separator), {@code 7} manufacturer, {@code 9} year. Other captured-but-unconfirmed fields
 * (status/type/ownership enums, one unexplained numeric field) aren't surfaced as columns, only in
 * {@code raw_response}.
 */
@Component
public class VektorTrailerMapper {
    ...
    public VektorTrailerRow toRow(VektorGrpcWeb.Message trailer) {
        ...
        return new VektorTrailerRow(id, label, manufacturer, year, writeAsJson(trailer), null);
    }
