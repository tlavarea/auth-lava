...
public class VektorSyncTasklet implements Tasklet {
    ...
    public @Nullable RepeatStatus execute(StepContribution contribution, ChunkContext chunkContext) {
        String jwt = vektorAuthenticator.authenticate();
        log.info("execute::vektor login successful");
        String companyId = vektorProperties.companyId();

        List<VektorGrpcWeb.Message> manifests =
                vektorManifestClient.fetchManifests(jwt, companyId, vektorProperties.syncedStatuses());
        Map<String, String> driverNamesById = vektorDriverClient.fetchDriverNamesById(jwt, companyId);
        List<SamsaraDriverRow> samsaraDrivers = samsaraDriverRepository.findAll();

        List<VektorManifestRow> rows = manifests.stream()
                .map(manifest -> vektorManifestMapper.toRow(manifest, driverNamesById))
                .map(row -> matchSamsaraDriver(row, samsaraDrivers))
                .toList();

        vektorManifestRepository.replaceAll(rows);
    ...
    private VektorManifestRow matchSamsaraDriver(VektorManifestRow row, List<SamsaraDriverRow> samsaraDrivers) {
        return vektorDriverMatchStrategy
