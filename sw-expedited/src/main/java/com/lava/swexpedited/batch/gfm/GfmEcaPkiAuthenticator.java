...
@Component
@Slf4j
public class GfmEcaPkiAuthenticator extends RetryingHttpClient {
    ...
    public void authenticate(URI mpsKmisAuthorizeUri) {
        ...
        String samlAutoSubmitPage = retrying(
                () -> this.gfmRestClient.get().uri(ecaPkiUri).retrieve().body(String.class), this.retryBackoff);
