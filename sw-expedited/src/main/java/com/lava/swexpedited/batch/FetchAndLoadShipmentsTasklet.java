...
@Component
@Slf4j
public class FetchAndLoadShipmentsTasklet implements Tasklet {
    ...
    public @Nullable RepeatStatus execute(StepContribution contribution, ChunkContext chunkContext) {
        URI teamsLoginUri = UriComponentsBuilder.fromUriString(gfmProperties.etaTeamsBaseUrl())
                .path("/teams/api/oauth/login")
                .queryParam("loginType", "certificate")
                .build()
                .toUri();
        retrying(() -> gfmRestClient.get().uri(teamsLoginUri).retrieve().toBodilessEntity());
        log.info("execute::teams login successful");
        logCookieNames("after teams login");

        URI gfmAuthorizeUri = UriComponentsBuilder.fromUriString(gfmProperties.mpsKmisBaseUrl())
                .path("/oauth2/authorize")
                .queryParam("response_type", "code")
                .queryParam("scope", "openid")
                .queryParam("client_id", gfmProperties.clientId())
                .queryParam("redirect_uri", gfmProperties.gfmBaseUrl() + "/gfmgateway/GfmGateway")
                .build()
                .toUri();
        gfmEcaPkiAuthenticator.authenticate(gfmAuthorizeUri);
        log.info("execute::gfm login successful");
        logCookieNames("after gfm login");

        URI atrHomeUri = UriComponentsBuilder.fromUriString(gfmProperties.gfmBaseUrl())
                .path("/atr/home")
                .build()
                .toUri();
        ...
        String atrHomeReferer = gfmGatewayReferer.get() != null
                ? gfmGatewayReferer.get()
                : gfmProperties.gfmBaseUrl() + "/gfmgateway/GfmGateway";
        log.info("execute::atrHomeReferer: {}", atrHomeReferer);
        ...
        retrying(() -> GfmFetchMetadataHeaders.forDocumentNavigation(gfmRestClient
                        .post()
                        .uri(atrHomeUri)
                        .header(HttpHeaders.REFERER, atrHomeReferer)
                        .header(HttpHeaders.ORIGIN, gfmProperties.gfmBaseUrl())
        ...
        URI shipmentCsvUri = UriComponentsBuilder.fromUriString(gfmProperties.gfmBaseUrl())
                .path("/atr/shipment")
                ...
                .build()
                .toUri();
        byte[] csv = retrying(() -> GfmFetchMetadataHeaders.forDocumentNavigation(gfmRestClient
                        .get()
                        .uri(shipmentCsvUri)
        ...
        List<ShipmentListingRow> rows = shipmentCsvParser.parse(csv);
        shipmentListingRepository.replaceAll(rows);
    ...
    private void logCookieNames(String checkpoint) {
        String cookies = gfmCookieStore.getCookies().stream()
                .map(cookie -> cookie.getName() + "@" + cookie.getDomain())
                .collect(Collectors.joining(", "));
        log.debug("execute::cookies present {}: {}", checkpoint, cookies);
    }
