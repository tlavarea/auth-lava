...
public class GfmLogoutJobListener implements JobExecutionListener {
    ...
    public void afterJob(JobExecution jobExecution) {
        log.info("afterJob::id: {} | exitStatus: {}", jobExecution.getJobInstanceId(), jobExecution.getExitStatus());

        Optional<String> csrfToken = gfmCookieStore.getCookies().stream()
        ...
        if (csrfToken.isPresent()) {
            try {
                URI gfmLogoutUri = UriComponentsBuilder.fromUriString(gfmProperties.gfmBaseUrl())
        ...
        try {
            URI mpsLogoutUri = UriComponentsBuilder.fromUriString(gfmProperties.mpsKmisBaseUrl())
                    .path("/oidc/logout")
                    .build()
                    .toUri();
            gfmRestClient.post().uri(mpsLogoutUri).retrieve().toBodilessEntity();
            log.info("afterJob::MPS logout successful");
        } catch (Exception e) {
            log.error("afterJob::MPS logout failed", e);
        }

        gfmCookieStore.clear();
